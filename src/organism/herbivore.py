...
class Herbivore:
    ...
    def aimed_move(self, aimed_herbivores):
        aimed_herbivores["x"] = aimed_herbivores["x"] + (
            ((aimed_herbivores["target_x"] - aimed_herbivores["x"]) / 2)
            * (aimed_herbivores["size"] * aimed_herbivores["speed"])
        )
        aimed_herbivores["y"] = aimed_herbivores["y"] + (
            ((aimed_herbivores["target_y"] - aimed_herbivores["y"]) / 2)
            * (aimed_herbivores["size"] * aimed_herbivores["speed"])
        )
    ...
    def move(self):
        aimless_herbivores = self.herbivore_population.loc[
            self.herbivore_population["target_x"] == -100
        ]
    ...
    def eat(self, plants):
        ...
        hungry_herbivores = self.herbivore_population.loc[
            ~self.herbivore_population.index.isin(eaten_plants.index)
        ]
        eating_herbivores = self.herbivore_population.loc[
            self.herbivore_population.index.isin(eaten_plants.index)
        ]
        eating_herbivores["energy"] = eating_herbivores["energy"] + eaten_plants["size"]
    ...
    def select_target(self, hungry_herbivores, plants):
        ...
        targetted_herbivores = hungry_herbivores.loc[
            hungry_herbivores.index.isin(nearby_plants.index)
        ]
        far_away_plants = plants.loc[~plants.index.isin(nearby_plants.index)]
        non_targetted_herbivores = hungry_herbivores.loc[
            hungry_herbivores.index.isin(far_away_plants.index)
        ]
        already_targetted = targetted_herbivores.loc[targetted_herbivores['target_x'] != -100]
        targetted_herbivores = targetted_herbivores.loc[targetted_herbivores.index.isin(already_targetted.index)]
        ...
        hungry_herbivores = pd.concat(
            [non_targetted_herbivores, targetted_herbivores, already_targetted]
        ).reset_index(drop=True)
        return hungry_herbivores

    def age(self):
        self.herbivore_population["age"] = self.herbivore_population["age"] + 1
        self.herbivore_population["energy"] = self.herbivore_population["energy"] - (
            (self.herbivore_population["speed"] + self.herbivore_population["size"]) / 4
        )
        self.reproduce()
        self.kill_old()
        self.starve()
